...
В обучение идут все 400 behaviors HarmBench. Раньше здесь был только официальный
`val` (80), а `test` (320) лежал в холде как кандидат в OOD-набор; после того как
LODO переехала на ToxicChat, резерв стал не нужен, а на 80 затравках каждая
переиспользовалась в ~54 парах. Вернуть холд можно через `train_split: val` и
`heldout_split: test` в [configs/rq2_pairs.yaml](configs/rq2_pairs.yaml).
